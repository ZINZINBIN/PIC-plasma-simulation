...
from src.utils import SORsolver, compute_hamiltonian
...
class PICsolver:
    def __init__(
        ...
        self.A_mesh     = np.zeros((N_mesh, N_mesh))
        ...
        # initialize x,v,a that is equivalent to the problem
        self.initialize_condition() 

        # index paramters for updating each mesh grid
        self.indx_l = np.zeros((N,2))

        self.indx_l[:,0] = np.floor(self.x[:,0] / self.dx).astype(int)
        self.indx_l[:,1] = np.floor(self.x[:,1] / self.dy).astype(int)
        self.indx_r = self.indx_l + np.ones_like(self.indx_l)

        self.weight_l = np.zeros_like(self.indx_l)
        self.weight_l[:,0] = (self.indx_r[:,0] * self.dx - self.x[:,0]) / self.dx
        self.weight_l[:,1] = (self.indx_r[:,1] * self.dy - self.x[:,1]) / self.dy

        self.weight_r = np.zeros_like(self.weight_l)
        self.weight_r[:,0] = (self.x[:,0] - self.indx_l[:,0] * self.dx) / self.dx
        self.weight_r[:,1] = (self.x[:,1] - self.indx_l[:,1] * self.dy) / self.dy
        ...
        # 2D electron density 
        '''
            1D: bincount + interpolation to estimate the density
            2D: how to define the number of electron in a 2D mesh grid
        '''    
